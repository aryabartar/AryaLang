...
(define numex-filter (lam "filter" "f"
                          (lam null "xs"
                               (ifmunit "xs"
                                        (munit)
                                        (with* (list (cons "1st" (apply "f" (1st "xs")))
                                                     (cons "2nd" (apply (apply "filter" "f") (2nd "xs"))))
                                               (ifnzero "1st" (apair "1st" "2nd") "2nd"))))))

(eval-exp(apply (apply numex-filter (lam null "x" (plus (num 1) (var "x"))))
